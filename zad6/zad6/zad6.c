...
int printReceiptsAndArticles(receiptPosition receiptHead) {
	receiptPosition receiptCurrent = receiptHead->next;


	while (receiptCurrent != NULL) {
		printf("\nRacun (%s):\n", receiptCurrent->receiptDate);

		articlePosition articleCurrent = receiptCurrent->articleHead.next;

		while (articleCurrent != NULL) {
			printf("  %-20s %5d kom  %5d EUR\n",
				articleCurrent->name,
				articleCurrent->quantity,
				articleCurrent->price);
